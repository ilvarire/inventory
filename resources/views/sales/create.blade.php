                    ...
                    <div class="mb-3 grid grid-cols-1 gap-3 md:grid-cols-3">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-900 dark:text-white">
                                Sale Date & Time <span class="text-red-500">*</span>
                            </label>
                            <input type="datetime-local" x-model="formData.sale_date" required
                                ...
                                <div class="flex gap-3">
                                    ...
                                    <div class="flex-1">
                                        <div x-data="{
                                            ...
                                            <input type="text" x-model="search" @input="filterProducts(); open = true"
                                                @click="open = true" @focus="open = true" placeholder="Search product..."
                                                class="w-full rounded border border-gray-300 bg-transparent px-5 py-3 text-gray-900 outline-none transition focus:border-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white" />
        ...
        <script>
            function recordSaleData() {
                return {
                    ...
                    formData: {
                        section_id: '',
                        sale_date: '', // Replaced with local time below
                        payment_method: '',
                        customer_name: '',
                        total_amount: 0,
                        sold_by: {{ auth()->id() }},
                    ...
                    async init() {
                        ...
                        @endif
                                    // Initialize date with local time
                                    const now = new Date();
                        const offset = now.getTimezoneOffset() * 60000; // in ms
                        const localISOTime = new Date(now - offset).toISOString().slice(0, 16);
                        this.formData.sale_date = localISOTime;
                    },
